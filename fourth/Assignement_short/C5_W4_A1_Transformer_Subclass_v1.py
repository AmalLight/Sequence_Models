...
def get_angles(pos, k, d):
    ...
    # print( np.sum( angles[:, 0] ) )
    
    # print( angles.shape[ 0 ] * ( angles.shape[ 0 ] - 1 ) / 2 )
...
def positional_encoding(positions, d):
    ...
    angle_rads = get_angles(np.array ( [ range ( 0 , positions ) ] ).T,
                            np.array ( [ range ( 0 , d ) ] ),
                            d)
    ...
    # print ( tf.cast(pos_encoding, dtype=tf.float32).shape )
...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    # print( output , output.shape , tf.is_tensor( output ) )
    # print( attention_weights , attention_weights.shape , tf.is_tensor( attention_weights ) )
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        attn_output = self.mha ( query=x , value=x , attention_mask=mask, training=training , return_attention_scores=False ) # Self attention (batch_size, input_seq_len, fully_connected_dim)
        ...
        # print ( encoder_layer_out , encoder_layer_out.shape )
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        mult_attn_out1, attn_weights_block1 = self.mha1( query=x , value=x , attention_mask=look_ahead_mask, training=training , return_attention_scores=True)  # (batch_size, target_seq_len, d_model)
