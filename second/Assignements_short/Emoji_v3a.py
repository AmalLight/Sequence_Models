...
for idx in range(10): print(X_train[idx], label_to_emoji(Y_train[idx]))
...
def sentence_to_avg(sentence, word_to_vec_map):
    ...
    # print ( word_to_vec_map [ ( sentence.lower() ).split( ' ' )[ 0 ] ].shape )
    # avg = np.array ( [ word_to_vec_map [ w ] * 0 for w in words if w in word_to_vec_map ] )
    # print ( any_word ) ; return 0
    ...
    avg = np.zeros ( np.array ( word_to_vec_map [ any_word ] ).T.shape )
            
    # Initialize count to 0 numpy.average(a, axis=None, weights=None, returned=False)
    count = 0
    ...
    for w in words:
        ...
        if w in word_to_vec_map:
            ...
            avg += np.array ( word_to_vec_map [ w ].T )
